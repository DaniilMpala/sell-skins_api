...
- `tradeUrl` (необязательный, но елательный): передавайте его в связке с стимайди, для того чтобы поулчить все доступные трейды для обмена юзера, иначе можем неувидеть предметы котоыре вышли из трейд бана за последние 3 дня (правила стима при просмотре инвентаря через стим айди) - трейд ссылка должна совпадать с SteamID передаваемым
...
- `11` : Трейд ссылка принадлежит не тому же юзеру который создавал транзакцию
