...
- `tradeUrl` (необязательный, но желательный): передавайте его в связке с стимайди, для того чтобы получить все доступные трейды для обмена юзера, иначе можем не увидеть предметы которые вышли из трейд бана за последние 3 дня (правила стима при просмотре инвентаря через стим айди) - трейд ссылка должна совпадать с SteamID передаваемым
